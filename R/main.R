...
#'
#' @param data Seurat object containing a query data set - filtering will be
#'     applied to this object
#' @param model A single scGate model, or a list of scGate models. See Details
#'     for this format
...
#' @param reduction Dimensionality reduction to use for knn smoothing. By
#'     default, calculates a new reduction
#'     based on the given \code{assay}; otherwise you may specify a
#'     precalculated dimensionality reduction (e.g. 
...
#' @param param_decay Controls decrease in parameter complexity at each
#'     iteration, between 0 and 1.
#'     \code{param_decay == 0} gives no decay, increasingly higher
#'     \code{param_decay} gives increasingly stronger decay
...
#' @param additional.signatures A list of additional signatures, not included
#'     in the model, to be evaluated (e.g. a cycling signature). The scores for
#'     this list of signatures will be returned but not used for filtering.
#' @param save.levels Whether to save in metadata the filtering output for each
#'     gating model level
#' @param by.knn Perform k-nearest neighbor smoothing for UCell scores
#' @param keep.ranks Store UCell rankings in Seurat object. This will speed up
#'     calculations if the same object is applied again with new signatures.
#' @param genes.blacklist Genes blacklisted from variable features. The default
#'     loads the list of genes in \code{scGate::genes.blacklist.default};
#'     you may deactivate blacklisting by setting \code{genes.blacklist=NULL}
#' @param multi.asNA How to label cells that are "Pure" for multiple
#'     annotations: "Multi" (FALSE) or NA (TRUE)
#' @param verbose Verbose output

#' @return A new metadata column \code{is.pure} is added to the query Seurat
#'     object, indicating which cells correspond to the desidered cell type(s).
#'     The \code{active.ident} is also set to this variable.
#' @details Models for scGate are dataframes where each line is a signature for
#'     a given filtering level. A database of models can be downloaded using the
#'     function \code{get_scGateDB}. You may directly use the models from the
#'     database, or edit one of these models to generate your own custom gating
#'     model. Multiple models can also be evaluated at once, by running scGate
#'     with a list of models. Gating for each individual model is returned as
#'     metadata, with a consensus annotation stored in \code{scGate_multi}
#'     metadata field. This allows using scGate as a multi-class classifier,
#'     where only cells that are "Pure" for a single model are assigned a label,
#'     cells that are "Pure" for more than one gating model are labeled as
#'     "Multi", all others cells are annotated as NA.
#' @examples
#' library(scGate)
...
#' seurat_object <- scGate(seurat_object, model=model.list, verbose=TRUE)
...
scGate <- function(data,
    ...
    if (assay == "integrated") { #UCell should not run on integrated assay
        if ('RNA' %in% Assays(data)) {
            ...
            stop("Cannot find assays with unintegrated data ",
    ...
    if (is.null(reduction) || tolower(reduction) == "calculate") {
        ...
        if (!reduction %in% Reductions(data)) {
            stop("Could not find reduction ",
                 reduction,
                 " in this object. Set reduction='calculate' to compute a new dimred")
        }
    ...
    if (verbose) {
        message("Computing UCell scores for all signatures using ",
                assay.ucell,
                " assay...\n")
    }
    data <- score.computing.for.scGate(data, model, ncores= ncores,
                                       assay=assay.ucell, slot=slot,
                                       maxRank=maxRank, keep.ranks=keep.ranks,
    ...
    for (m in names(model)) {
        ...
        data <- run_scGate_singlemodel(data, model=model[[m]], k.param=k.param,
                                       ...
                                       resol=resol, nfeatures=nfeatures,
                                       by.knn=by.knn, min.cells=min.cells,
    ...
    data <- combine_scGate_multiclass(data,
                                      prefix=paste0(output.col.name,"_"),
                                      scGate_classes=names(m),
...
plot_tree <- function(model, box.size=8, edge.text.size=4) {
    ...
    if(nlev <= 1){
        stop("your model must contain at least two levels ",
             "to be ploted as a tree")
    }
    ...
    colnames(df) <- c(paste0("Level_", seq_len(nlev)), "Pure")
    
    for (i in seq_len(nlev-1)+1) {
...
get_scGateDB <- function(destination="./scGateDB",
    ...
    allfiles <- list.files(repo_path.v, recursive=TRUE, full.names=TRUE)
    ...
    for (dir in uniq_dirs) {
        ...
        tmp <- gsub(pattern, "", dir, perl=TRUE)
        ...
        if (length(sub) == 1) {
            ...
            message("Warning: max depth for scGate models is 3. ",
                    "Skipping folder ",
                    dir)
        } 
    }
...
#' @examples
#' results <- performance_metrics(actual=sample(c(1,0),20,replace=TRUE),pred=sample(c(1,0),20,replace=TRUE,prob=c(0.65,0.35) ) )
#' @export

performance_metrics <- function(actual, pred, return_contingency=FALSE){
...
test_my_model <- function(model,testing.version='hsa.latest', custom.dataset=NULL,target=NULL, plot=TRUE){
    ...
    if(!custom){
        ...
        if(is.null(target)){
            message("Warning: target cell_type not provided. ",
                    "Avoiding performance computation")
            performace.computation <- FALSE
        }else if(!target %in% targets){
            stop("target must be one of ",
                 paste(targets,collapse="';'"),
                 "; or NULL for avoiding performance computation")
        }
        
        ## check dataset version
        available.datasets <- c("hsa.latest")
        if(!testing.version %in% available.datasets){
            stop("please, provide a valid testing.version paramter ",
                 "or provide a custom.dataset in seurat format")
        }
    ...
    if(custom){
        if(!"cell_type" %in% colnames(custom.dataset@meta.data)){
            stop("please, provide a 'cell_type' column to be used ",
                 "as reference cell type")
        }
        ...
        if(is.null(target)){
            ...
            stop("all target celltypes must be included in ",
                 "cell_type metadata field. Otherwise, ",
                 "set target=NULL for avoiding performance computation")
        }
    }
    ...
    for(dset in names(testing.datasets)){
        ...
        plt <- list()
        dropcols <- obj@meta.data %>% colnames() %>% grep("^is.pure",.,value =TRUE) %>% unique()
        ...
        plt <- DimPlot(obj, group.by="cell_type",label=TRUE, repel=TRUE,label.size=3) + 
...
#' # Apply scGate with this model
#' query <- scGate(query, model=model, save.levels=TRUE)
#' # View UCell score distribution
#' plot_UCell_scores(query, model)
#' @return Either a plot combined by patchwork (combine=TRUE) or a list of plots (combine=FALSE)
#' @import ggplot2
...
#' @export
#' 
plot_UCell_scores <- function(obj, model, overlay=5, pos.thr=0.2, neg.thr=0.2, ncol=NULL, combine=TRUE) {
    ...
    if (sum(grepl("is.pure.level", colnames(obj@meta.data))) == 0) {
         ...
         if (length(levs)>1) {
             warning("scGate levels were not stored in this object. ",
                     "Showing results only for top level.")
             levs <- "level1"
         }
    }
    ...
    for (l in seq_along(levs)) {
        ...
        if (!is.null(pos.thr) | !is.null(neg.thr)) {
            pll[[l]] <- pll[[l]] + geom_segment(aes(x=.data$Thr, xend=.data$Thr,
                                                    ...
                                                    yend=as.numeric(.data$Signature)+0.9),
                                                linetype="dashed")
        }
    }
...
#'                          level=1, positive=TRUE, name="immune", signature=c("PTPRC"))
#' my_model <- gating_model(model=my_model, 
#'                          level=1, positive=FALSE, name="Epithelial", signature=c("CDH1","FLT1") )
#' # Remove an existing signature
#' dropped_model <- gating_model(model=my_model, remove=TRUE, level=1, name="Epithelial")
#' @importFrom stats setNames
#' @export

gating_model <- function(model=NULL, level= 1, name, signature, positive=TRUE, negative=FALSE, remove=FALSE){
    ...
    if(!remove){
        new.signature <- data.frame(levels=paste0("level",level),
                                    ...
                                    name=name,
                                    signature=ifelse(length(signature) >1,
