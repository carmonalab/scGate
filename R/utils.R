run_scGate_singlemodel <- function(data, model, pos.thr=0.2, neg.thr=0.2, 
                                   assay=NULL, slot="data",
                                   reduction="calculate", nfeatures=2000, 
                                   pca.dim=30, resol=3, param_decay=0.25,
                                   min.cells=30, by.knn=TRUE, k.param=10,
    ...
    for (lev in seq_along(list.model)) {
        if (verbose) {
            message("Running scGate on level ", lev, "...")
        }
        ...
        q <- find.nn(q, by.knn=by.knn, assay=assay, slot=slot,
                     min.cells=min.cells, nfeatures=nfeat.use,
                     reduction=reduction, npca=pca.use,
        ...
        q <- filter_bymean(q, positive=pos.names, negative=neg.names,
                           pos.thr=pos.thr, assay=assay, min.cells=min.cells,
                           neg.thr=neg.thr, by.knn=by.knn)
        
        n_rem <- sum(q$is.pure == "Impure")
        frac.to.rem <- n_rem/tot.cells
        mess <- sprintf("scGate: Detected %i non-pure cells at level %i", 
                        n_rem, lev)
        ...
        if(any(retain_pure_cells)){
            output_by_level[names(retain_pure_cells[retain_pure_cells == TRUE]),lev] <- "Pure" # save layer output
    ...
    if(any(retain_pure_cells)){
        ...
        message("Warning, all cells were removed at level ",
                lev,
                ". Consider reviewing signatures or model layout...")
    }
...
find.nn <- function(q, assay="RNA", slot="data", npca=30, nfeatures=2000,
                    k.param=10,min.cells=30, by.knn=FALSE, reduction="calculate",
    ...
    if (reduction == "calculate") {
        ...
        if (ngenes > 200) {    #only perform this for high-dim data
            q <- FindVariableFeatures(q, selection.method="vst",
        ...
        q <- RunPCA(q, features=q@assays[[assay]]@var.features, npcs=npca,
                    verbose=FALSE, reduction.key="knnPCA_")
    ...
    q <- suppressMessages(FindNeighbors(q, reduction=red.use, dims=1:npca,
...
## Filter by mean
filter_bymean <- function(q, positive, negative, pos.thr=0.1, neg.thr=0.2,
                          min.cells=30, assay="RNA", return_object=TRUE,
                          by.knn=FALSE) {
    
    DefaultAssay(q) <- assay
    ncells <- dim(q)[2]
    notfound <- c(positive[!positive %in% colnames(q@meta.data)],
                  negative[!negative %in% colnames(q@meta.data)])
    
    if (length(notfound)>0) {
        message("Warning: signatures not found: ", notfound)
    }
...
score.computing.for.scGate <- function(data, model, ncores=1, assay="RNA",
                                       slot="data", add.sign=NULL, 
                                       keep.ranks=FALSE, maxRank=1500) {
    
    comb <- dplyr::bind_rows(model, .id="Model_ID")
    # extract unique signatures
    model.uniq <- comb %>% dplyr::distinct(.data$name, .data$signature, .keep_all=TRUE) 
    ...
    if (length(dup)>0) {
        ...
        stop("Different gene sets have been assigned to signature with same name: ", s)
    }
    ...
    data <- UCell::AddModuleScore_UCell(data, features=signatures, assay=assay,
                                        slot=slot, ncores=ncores,
                                        storeRanks=keep.ranks, maxRank=maxRank)
...
table.to.model <- function(scGate.table){
    mod <- list()
    for(i in seq_len(nrow(scGate.table))){ 
        lev <- scGate.table$levels[i] 
        useas <- tolower(scGate.table$use_as[i])
        if(!useas %in% c("positive","negative")){
            message("Error: row ",
                    i,
                    " do not contain neither, 'positive' or 'negative' strings in 'use_as' column")
            return(NULL)
        }
...
load.model.helper <- function(models_path, master.table="master_table.tsv") {
    ...
    if(length(files.to.impute) == 0){
        stop("Please, provide some model table files in your 'model folder' ",
             "or set models_path=NULL for using the default ones")
    }
    # load models to impute
    for(f in files.to.impute){
        model.name <- strsplit(f,"_scGate_Model.tsv")[[1]][1]
        df.models.toimpute[[model.name]] <- read.table(file.path(models_path,f),sep="\t",header=TRUE)
    }
    # signature imputing
...
use_master_table <- function(df.model, master.table, name="name",descript="signature"){
    ...
    if(!file.exists(master.table)){
        stop("master_table.tsv file must be present in your 'model folder' ",
             "unless signatures are completely specified in the models")
    }
    master.table <- read.table(master.table, sep="\t", header=TRUE) 
    ...
    if(length(warn)>0){
        stop("signatures '",
             paste(warn,collapse=" ; " ),
             "' are not present in the provided master.signature table")
    }
    
    merged <- merge(df.model[complete.from.master.table,],master.table,by=name,suffixes=c("","_from.master"),all.x=TRUE)
